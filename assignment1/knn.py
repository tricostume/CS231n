...
print('Difference was: %f, sucker!!!' % (difference, ))
if difference < 0.001:
...
k_to_accuracies = {}
fold_level_accuracies = np.zeros(num_folds)
...
for j in range(len(k_choices)): 
    ...
    for i in range(num_folds):
        ...
        del(X_train_folds[i])
        del(y_train_folds[i])
        X_train_folds_temp = np.vstack(X_train_folds)
        y_train_folds_temp = np.hstack(y_train_folds)
        X_train_folds.insert(i,X_test_temp)
        y_train_folds.insert(i,y_test_temp)
        # Start training
        classifier.train(X_train_folds_temp, y_train_folds_temp)
        dists_two_temp = classifier.compute_distances_no_loops(X_test_temp)
        y_test_pred = classifier.predict_labels(dists_two_temp, k)
        num_correct = np.sum(y_test_pred == y_test_temp)
        accuracy = float(num_correct) / num_test_temp
        fold_level_accuracies[i] = accuracy
    k_to_accuracies[k] = fold_level_accuracies.tolist()
################################################################################
#                                 END OF YOUR CODE                             #
################################################################################
