...
# design so that we can implement different layer types in isolation and then 
...
#modular approach. For each layer we will implement a `forward` and a `backward`
...
#we will also explore different update rules for optimization, and introduce 
#Dropout as a regularizer and Batch Normalization as a tool to more efficiently
...
# Now implement the `affine_backward` function and test your implementation 
...
# nets. For example, affine layers are frequently followed by a ReLU 
#nonlinearity. To make these common patterns easy, we define several 
#convenience layers in the file `cs231n/layer_utils.py`.
# 
...
#networks you will implement in this assignment, so read through it to make 
#sure you understand the API. You can run the cell below to test your implementation.
...
#  models themselves. Following a more modular design, for this assignment we 
#have split the logic for training models into a separate class.
# 
...
model = model = TwoLayerNet(hidden_dim=150)
solver = None
...
#You will need to tweak the learning rate and initialization scale, but you 
#should be able to overfit and achieve 100% training accuracy within 20 epochs.
...
solver = Solver(model, small_data,
                print_every=10, num_epochs=60, batch_size=25,
                update_rule='sgd',
                optim_config={
...
#50 training examples. Again you will have to adjust the learning rate and weight 
#initialization, but you should be able to achieve 100% training accuracy 
#within 20 epochs.
...
#three-layer net vs training the five layer net?
# 
# # Answer:
# [FILL THIS IN]
# The deeper the network the harder it is to train. Also the initialization
# determines the difference between being able to learn or not. The weight 
# initialization must be more finely granular adequated than the learning rate.

# # Update rules
# So far we have used vanilla stochastic gradient descent (SGD) as our 
#update rule. More sophisticated update rules can make it easier to train deep
# networks. We will implement a few of the most commonly used update rules and 
#compare them to vanilla SGD.
...
# 
# Open the file `cs231n/optim.py` and read the documentation at the top of the
# file to make sure you understand the API. Implement the SGD+momentum update 
#rule in the function `sgd_momentum` and run the following to check your 
...
#`rmsprop` function and implement the Adam update rule in the `adam` function, 
#and check your implementations using the tests below.
# 
...
#and `Dropout.ipynb` notebooks before completing this part, since those 
#techniques can help you train powerful models.
    ...
    for rate in learning_rates:
        for scale in weight_scales:
            for hidden_size in hidden_sizes :
                for num_layer in num_hid_layers:
                  print('Running combination [ ', rule, rate, scale, hidden_size, num_layer, ' ]')
                  model = FullyConnectedNet([hidden_size]*num_layer, weight_scale = scale)
                  ...
                  if solver.val_acc_history[-1] > best_val:
                      ...
                      best_model = model
                      best_combination = [rule, rate, scale, hidden_size, num_layer]
                  print()
################################################################################
#                              END OF YOUR CODE                                #
################################################################################
from PIL import Image
jpgfile = Image.open("doggie.jpg")
jpgfile = list(jpgfile.getdata())
doggie = np.array(jpgfile).reshape(32,32,3)
plt.imshow(doggie.astype('uint8'))
doggie = doggie[np.newaxis,:]
prediction = best_model.loss(doggie)
winclass = np.argmax(prediction)
# # Test you model
